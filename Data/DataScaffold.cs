...
public static class DataScaffold
{
    ...
    public static IReadOnlyList<string[]> BuildListRows(DataEntityMetadata metadata, IEnumerable items, string basePath, bool includeActions, Func<DataEntityMetadata, bool>? canRenderLookupLink = null, string? cloneToken = null, string? cloneReturnUrl = null)
    {
        ...
        foreach (var item in items)
        {
            ...
            if (includeActions && item is BaseDataObject dataObject)
            {
                ...
                {
                    cloneHtml = BuildCloneActionHtml($"{basePath}/{safeId}/clone", cloneToken, cloneReturnUrl, "Clone row", "bi-files", "btn-outline-primary")
                        + BuildCloneActionHtml($"{basePath}/{safeId}/clone-edit", cloneToken, cloneReturnUrl, "Clone and edit", "bi-pencil-square", "btn-outline-warning");
                }
    ...
    private static string BuildCloneActionHtml(string actionUrl, string token, string? returnUrl, string title, string iconClass, string buttonClass)
    {
        var safeAction = WebUtility.HtmlEncode(actionUrl);
        var safeToken = WebUtility.HtmlEncode(token);
        var safeReturnUrl = WebUtility.HtmlEncode(returnUrl ?? string.Empty);
        return $"<form class=\"d-inline\" method=\"post\" action=\"{safeAction}\"><input type=\"hidden\" name=\"{CsrfProtection.FormFieldName}\" value=\"{safeToken}\" /><input type=\"hidden\" name=\"returnUrl\" value=\"{safeReturnUrl}\" /><button type=\"submit\" class=\"btn btn-sm {WebUtility.HtmlEncode(buttonClass)} me-1\" title=\"{WebUtility.HtmlEncode(title)}\" aria-label=\"{WebUtility.HtmlEncode(title)}\"><i class=\"bi {WebUtility.HtmlEncode(iconClass)}\" aria-hidden=\"true\"></i></button></form>";
    }
    ...
    internal static string[] ParseStringList(string rawValue)
    {
