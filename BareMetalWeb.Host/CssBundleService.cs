...
public static class CssBundleService
{
    ...
    /// Only the default theme (<c>darkly</c>) is loaded into memory eagerly;
    /// all other themes are loaded on first request.
    ///
    ...
    /// only the default theme (<c>darkly</c>) into memory.
    /// </summary>
    public static void DiscoverThemes(string cssDirectory)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(themesDir, "*.min.css"))
        {
            ...
            // Eagerly load the default theme so the first page load is instant
            if (fileName.Equals("darkly.min.css", StringComparison.OrdinalIgnoreCase))
