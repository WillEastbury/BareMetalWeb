...
public static class DataScaffold
{
    ...
    {
        list = null;
        var listFactory = ListFactoryCache.GetOrAdd(childType, t =>
        {
            var lt = typeof(List<>).MakeGenericType(t);
            return () => Activator.CreateInstance(lt)!;
        });
        var instanceFactory = InstanceFactoryCache.GetOrAdd(childType, t => () => Activator.CreateInstance(t)!);
    ...
    {
        dictionary = null;
        var dictFactory = DictFactoryCache.GetOrAdd((typeof(string), valueType), k =>
        {
            var dt = typeof(Dictionary<,>).MakeGenericType(k.Item1, k.Item2);
