...
public class BareMetalWebServer
{
    ...
    public List<MenuOption> MenuOptionsList {get;set;} = new List<MenuOption>();
    ...
    public StaticFileOptions StaticFiles { get; set; } = new();
    ...
    public void BuildAppInfoMenuOptions(HttpContext? context = null)
    {
        ...
        if (context != null)
        {
            ...
            {
                MenuOptionsList = new List<MenuOption>(cached.Options);
                return;
            }

            _menuCache.Remove(cacheKey);
        }
        foreach (var rte in routes.Where(kvp => kvp.Value.PageInfo != null && kvp.Value.PageInfo.PageMetaData.ShowOnNavBar))
        {
    ...
    private readonly record struct MenuCacheEntry(MenuOption[] Options, DateTime ExpiresUtc);
