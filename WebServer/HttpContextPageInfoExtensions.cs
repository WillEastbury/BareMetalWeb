...
public static class HttpContextPageInfoExtensions
{
    ...
    public static void SetApp(this HttpContext context, BareMetalWebServer app)
    {
        context.Items[AppKey] = app;
    }

    public static BareMetalWebServer? GetApp(this HttpContext context)
    {
        return context.Items.TryGetValue(AppKey, out var value) ? value as BareMetalWebServer : null;
    }
    ...
    {
        return context.GetPageContext() ?? new PageContext(Array.Empty<string>(), Array.Empty<string>());
    }
}
