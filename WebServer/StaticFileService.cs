...
public sealed class StaticFileOptions
{
    ...
    internal PhysicalFileProvider? FileProvider { get; private set; }
    internal MemoryCache MetadataCache { get; private set; } = new(new MemoryCacheOptions { SizeLimit = 50_000 });
    internal int MetadataCacheSizeLimit { get; private set; } = 50_000;

    public static StaticFileOptions FromConfig(StaticFileOptionsConfig? config)
    {
    ...
    public void Normalize()
    {
        ...
        foreach (var kvp in StaticFileService.DefaultMimeTypes)
        {
...
public static class StaticFileService
{
    ...
    internal static readonly Dictionary<string, string> DefaultMimeTypes = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    public static async Task<bool> TryServeAsync(HttpContext context, StaticFileOptions? options)
    {
    ...
    private static bool TryGetContentType(StaticFileOptions options, string fullPath, out string contentType)
    {
    ...
    private static bool TryGetFileMetadata(StaticFileOptions options, string fullPath, string relativeFullPath, string contentTypePath, string relativeContentTypePath, out FileMetadata metadata)
    {
        metadata = default;
    ...
    private static bool TryResolveDefaultFile(StaticFileOptions options, string rootPath, string relativePath, out string fullPath, out string resolvedRelative)
    {
    ...
    private static async Task WriteDirectoryListing(HttpContext context, string rootPath, string relativePath, StaticFileOptions options)
    {
    ...
    private static CompressionSelection SelectCompression(HttpContext context, StaticFileOptions options)
    {
    ...
    private static bool IsCompressibleContentType(StaticFileOptions options, string contentType)
    {
        if (string.IsNullOrWhiteSpace(contentType))
            return false;
