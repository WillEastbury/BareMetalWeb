...
static class ProgramSetup
{
    ...
    public static IDataObjectStore CreateDataStore(WebApplication app, ISchemaAwareObjectSerializer serializer, IDataQueryEvaluator queryEvaluator, IBufferedLogger logger)
    {
        ...
        if (checkpointEnabled && checkpointSeconds > 0)
        {
            DataStoreProvider.ClusteredCheckpointScheduler = new ClusteredCheckpointScheduler(
                provider,
    ...
    {
        var staticFileConfig = app.Configuration.GetSection("StaticFiles").Get<BareMetalWeb.WebServer.StaticFileOptionsConfig>()
            ?? new BareMetalWeb.WebServer.StaticFileOptionsConfig();
        var staticFileOptions = BareMetalWeb.WebServer.StaticFileOptions.FromConfig(staticFileConfig);
    ...
    public static void ConfigureProxyRoutes(WebApplication app, BareMetalWebServer appInfo, IBufferedLogger logger, IPageInfoFactory pageInfoFactory)
    {
