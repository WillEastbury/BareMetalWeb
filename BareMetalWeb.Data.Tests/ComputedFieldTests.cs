...
public class ComputedFieldTests : IDisposable
{
    ...
    [Fact]
    public void BuildFormFields_SnapshotFieldsExcludedFromCreate()
    {
        // Arrange
        DataScaffold.RegisterEntity<TestProduct>();
        DataScaffold.RegisterEntity<TestOrder>();
        ...
        // Act
        var createFields = DataScaffold.BuildFormFields(metadata, null, forCreate: true);

        // Assert - snapshot fields with OnCreate trigger should be excluded from create form
        var snapshotField = Array.Find(createFields.ToArray(), 
            f => f.Name == nameof(TestOrder.UnitPriceSnapshot));
        Assert.Null(snapshotField);
    }

    [Fact]
    public void BuildFormFields_ComputedFieldsShownAsReadOnlyInEdit()
    {
        // Arrange
        DataScaffold.RegisterEntity<TestProduct>();
        DataScaffold.RegisterEntity<TestOrder>();
        ...
        // Act
        var editFields = DataScaffold.BuildFormFields(metadata, order, forCreate: false);

        // Assert - computed fields should be rendered as readonly
        var snapshotField = Array.Find(editFields.ToArray(), 
            f => f.Name == nameof(TestOrder.UnitPriceSnapshot));
        Assert.NotNull(snapshotField);
        Assert.Equal(FormFieldType.ReadOnly, snapshotField.FieldType);
        Assert.True(snapshotField.IsComputed);
        Assert.Equal("Snapshot", snapshotField.ComputedStrategy);
    }

    [Fact]
    public async Task ApplyComputedValuesAsync_SnapshotStrategy_OnCreate()
    {
        // Arrange
        ...
        try
        {
            ...
            // Act - apply computed values for OnCreate trigger
            await DataScaffold.ApplyComputedFieldsAsync(metadata, order, ComputedTrigger.OnCreate);

            // Assert - snapshot field should be populated
            Assert.Equal(49.99m, order.UnitPriceSnapshot);
        }
        finally
        {
