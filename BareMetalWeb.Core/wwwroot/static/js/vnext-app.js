...
(function (global) {
    ...
    function renderTreeView(meta, items, slug, baseUrl) {
        ...
        if (items.length === 0) {
            html += '<p class="text-muted p-3 mb-0">No records found.</p>';
        } else if (parentField) {
            var roots = [], nodeMap = {};
            items.forEach(function (item) {
                var id = item.id || item.Id || '';
            ...
            items.forEach(function (item) {
                ...
                if (parentId && parentId !== id && nodeMap[parentId]) nodeMap[parentId].children.push(nodeMap[id]);
                else roots.push(nodeMap[id]);
            });
            if (roots.length === 0) {
                // Circular reference or all items are children — break cycle, show all as roots
