...
public class LookupApiHandlerTests : IDisposable
{
    ...
    public async Task BatchGetEntities_ReturnsMatchedEntities_WhenAllExist()
    {
        ...
        var context = CreatePostHttpContext("/api/_lookup/products/_batch", new Dictionary<string, object?> { ["ids"] = new[] { "1", "2" } });
    ...
    public async Task BatchGetEntities_OmitsMissingEntities_WhenSomeNotFound()
    {
        ...
        var context = CreatePostHttpContext("/api/_lookup/products/_batch", new Dictionary<string, object?> { ["ids"] = new[] { "1", "999" } });
    ...
    {
        // Arrange
        var context = CreatePostHttpContext("/api/_lookup/products/_batch", new Dictionary<string, object?> { ["ids"] = Array.Empty<string>() });
    ...
    {
        // Arrange
        var context = CreatePostHttpContext("/api/_lookup/products/_batch", new Dictionary<string, object?> { ["something"] = "else" });
    ...
    {
        // Arrange
        var context = CreatePostHttpContext("/api/_lookup/nonexistent-type/_batch", new Dictionary<string, object?> { ["ids"] = new[] { "1" } });
    ...
    public async Task BatchGetEntities_DeduplicatesIds_WhenDuplicatesPassed()
    {
        ...
        var context = CreatePostHttpContext("/api/_lookup/products/_batch", new Dictionary<string, object?> { ["ids"] = new[] { "1", "1", "1" } });
    ...
    public async Task BatchGetEntities_WithTraverseRelationships_ExpandsLookupFields()
    {
        ...
        var context = CreatePostHttpContext("/api/_lookup/orders/_batch?traverseRelationships=true", new Dictionary<string, object?> { ["ids"] = new[] { "401" } });
    ...
    private HttpContext CreatePostHttpContext(string path, Dictionary<string, object?> body)
    {
        var context = CreateHttpContext("POST", path);
        using var buffer = new MemoryStream();
        using (var w = new Utf8JsonWriter(buffer))
        {
            JsonWriterHelper.WriteValue(w, body);
        }
