...
public class HtmlRenderer : IHtmlRenderer
{
    ...
    public async ValueTask<byte[]> RenderToBytesAsync(IHtmlTemplate template, string[] keys, string[] values, string[] appkeys, string[] appvalues, BareMetalWebServer app, string[]? tableColumnTitles = null, string[][]? tableRows = null, FormDefinition? formDefinition = null, TemplateLoop[]? templateLoops = null)
    {
    ...
    public async ValueTask RenderToStreamAsync(PipeWriter writer, IHtmlTemplate template, string[] keys, string[] values, string[] appkeys, string[] appvalues, BareMetalWebServer app, string[]? tableColumnTitles = null, string[][]? tableRows = null, FormDefinition? formDefinition = null, TemplateLoop[]? templateLoops = null)
    {
    ...
    public async ValueTask RenderPage(HttpContext context)
    {
        ...
        if (user == null)
        {
            context.SetStringValue("footer_user", string.Empty);
        }
        else
        {
            var displayName = string.IsNullOrWhiteSpace(user.DisplayName) ? user.UserName : user.DisplayName;
            context.SetStringValue("footer_user", $"<span class=\"ms-2\">Signed in as <strong>{WebUtility.HtmlEncode(displayName)}</strong></span>");
        }
    ...
    public async ValueTask RenderPage(HttpContext context, PageInfo page, BareMetalWebServer app)
    {
