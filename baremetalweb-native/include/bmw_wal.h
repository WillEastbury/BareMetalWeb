...
/* Delta header: [key_hash:u32][value_len:u16][op:u8][reserved:u8] */
#pragma pack(push, 1)
typedef struct {
    uint32_t key_hash;
    uint16_t value_len;
...
typedef struct {
    ...
    int (*append)(void *engine, uint32_t key_hash, const uint8_t *value,
                  uint16_t value_len, wal_op_t op, uint32_t *out_seq);

    /* Read all deltas for a key */
    int (*read)(void *engine, uint32_t key_hash, uint8_t *out_buf,
